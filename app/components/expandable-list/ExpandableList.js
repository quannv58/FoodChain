...
export class ExpandableList extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.data.length != this.props.data.length) {
      const arr = Array(nextProps.data.length)
        .fill(false)
        .map((item, index) => {
          return nextProps.initialOpenGroups.indexOf(index) !== -1;
        });
      this.setState({groupStatus: arr});
    }
  }
